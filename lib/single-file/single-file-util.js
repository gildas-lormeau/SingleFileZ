		...
		getInstance: (modules, util) => {
			...
			return {
				...
				getFilenameExtension(resourceURL) {
					return (new URL(resourceURL)).pathname.match(/(\.[^.]*)$/).toLowerCase();
				},
				...
				if (options.asBinary) {
					try {
						...
						if (options.maxResourceSizeEnabled && resourceContent.getSize() > options.maxResourceSize * ONE_MB) {
							...
							return { data: Array.from(new Uint8Array(await resourceContent.getBuffer())), resourceURL, contentType };
						}
					} catch (error) {
						return { resourceURL };
					}
				} else {
					if (resourceContent.getStatusCode() >= 400 || (options.validateTextContentType && !contentType.startsWith("text/"))) {
						return { resourceURL };
					}
					if (!charset) {
						charset = "utf-8";
					...
					if (options.maxResourceSizeEnabled && resourceContent.getSize() > options.maxResourceSize * ONE_MB) {
						...
						try {
							return { data: resourceContent.getText(charset), resourceURL, charset, contentType };
						} catch (error) {
							try {
								charset = "utf-8";
								return { data: resourceContent.getText(charset), resourceURL, charset, contentType };
