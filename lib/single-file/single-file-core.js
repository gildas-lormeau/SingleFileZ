...
this.singlefile.lib.core = this.singlefile.lib.core || (() => {
	...
	class BatchRequest {
		...
		async addURL(resourceURL, asBinary) {
		...
		async run(onloadListener, options) {
			...
			return Promise.all(resourceURLs.map(async requestKey => {
				...
				try {
					...
					const extensionMatch = util.getFilenameExtension(resourceURL);
					resourceRequests.forEach(callbacks => {
						callbacks.resolve({ content: content.data, indexResource, contentType: content.contentType, extension: (extensionMatch && extensionMatch[1]) || "" });
					});
				} catch (error) {
					indexResource = indexResource + 1;
					onloadListener({ url: resourceURL });
	...
	// Processor
	// ---------
	const PREFIX_CONTENT_TYPE_IMAGE_SVG = "image/svg+xml";
	const PREFIX_DATA_URI_IMAGE_SVG = "data:" + PREFIX_CONTENT_TYPE_IMAGE_SVG + ",";
	...
	class Processor {
		...
		async getPageData() {
			...
			const resources = {};
			Object.keys(this.resources).forEach(resourceType => resources[resourceType] = Array.from(this.resources[resourceType].values()));
			return {
		...
		async processPageResources() {
			...
			let resourcePromises = processAttributeArgs.map(([selector, attributeName, removeElementIfMissing]) =>
				ProcessorHelper.processAttribute(this.doc.querySelectorAll(selector), attributeName, this.baseURI, this.resources, this.batchRequest, removeElementIfMissing)
			);
			...
			if (!this.options.removeAudioSrc) {
				resourcePromises.push(ProcessorHelper.processAttribute(this.doc.querySelectorAll("audio[src], audio > source[src]"), "src", this.baseURI, this.resources, this.batchRequest));
			}
			if (!this.options.removeVideoSrc) {
				resourcePromises.push(ProcessorHelper.processAttribute(this.doc.querySelectorAll("video[src], video > source[src]"), "src", this.baseURI, this.resources, this.batchRequest));
			}
		...
		async processScripts() {
			await Promise.all(Array.from(this.doc.querySelectorAll("script[src]")).map(async scriptElement => {
				...
				if (Util.testValidURL(resourceURL)) {
					...
					const name = "scripts/" + content.indexResource + ".js";
					scriptElement.setAttribute("src", name);
					this.resources.scripts.set(content.indexResource, { name, content: content.data });
				}
			}));
		}

		removeAlternativeImages() {
			util.removeAlternativeImages(this.doc, this.resources.images);
		}

		removeAlternativeFonts() {
			util.removeAlternativeFonts(this.doc, this.stylesheets, this.resources.fonts);
		}
		...
		replaceStylesheets() {
			...
			this.doc.querySelectorAll("link[rel*=stylesheet]").forEach(linkElement => {
				...
				if (stylesheetInfo) {
					...
					const name = "stylesheets/" + this.resources.stylesheets.size + ".css";
					linkElement.setAttribute("href", name);
					if (stylesheetInfo.mediaText) {
						linkElement.media = stylesheetInfo.mediaText;
					}
					this.resources.stylesheets.set(this.resources.stylesheets.size, { name, content: cssTree.generate(stylesheetInfo.stylesheet) });
				} else {
					linkElement.remove();
				}
			});
	...
	const PREFIX_CONTENT_TYPE_VND = "application/vnd.";
	const PREFIX_DATA_URI_VND = "data:" + PREFIX_CONTENT_TYPE_VND;
	...
	class ProcessorHelper {
				...
				await Promise.all(ruleData.block.children.toArray().map(async declaration => {
					if (declaration.type == "Declaration" && declaration.value.children) {
						...
						await Promise.all(urlFunctions.map(async urlFunction => {
							...
							if (!Util.testIgnoredPath(resourceURL)) {
								if (Util.testValidURL(resourceURL)) {
									let { content, extension, indexResource, contentType } = await batchRequest.addURL(resourceURL, true);
									if (content && contentType && !contentType.startsWith(PREFIX_CONTENT_TYPE_VND) && !contentType.startsWith(PREFIX_CONTENT_TYPE_IMAGE_SVG)) {
									...
									resources.fonts.set(indexResource, { name, content, extension, contentType });
								}
			...
			await Promise.all(declarations.map(async declaration => {
				if (declaration.type == "Declaration" && declaration.value.children) {
					...
					await Promise.all(urlFunctions.map(async urlFunction => {
						...
						if (!Util.testIgnoredPath(resourceURL)) {
							if (Util.testValidURL(resourceURL)) {
								let { content, indexResource, contentType, extension } = await batchRequest.addURL(resourceURL, true);
								const name = "background-images/" + indexResource + extension;
								findURLToken(originalResourceURL, declaration.value.children, token => {
									token.data.value.value = name;
								});
								resources.backgroundImages.set(indexResource, { name, content, extension, contentType });
							}
						}
					}));
				}
		...
		static async processAttribute(resourceElements, attributeName, baseURI, resources, batchRequest, removeElementIfMissing) {
			await Promise.all(Array.from(resourceElements).map(async resourceElement => {
				...
				if (!Util.testIgnoredPath(resourceURL)) {
					...
					if (Util.testValidPath(resourceURL)) {
						...
						if (Util.testValidURL(resourceURL)) {
							const { content, indexResource, extension, contentType } = await batchRequest.addURL(resourceURL, true);
							if (removeElementIfMissing && !content) {
								...
								resources.images.set(indexResource, { name, content, extension, contentType });
							}
						}
					}
				}
		...
		static async processSrcset(resourceElements, attributeName, baseURI, resources, batchRequest) {
			await Promise.all(Array.from(resourceElements).map(async resourceElement => {
