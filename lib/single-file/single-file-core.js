		...
		replaceStylesheets() {
			...
			this.doc.querySelectorAll("link[rel*=stylesheet]").forEach(linkElement => {
				...
				if (stylesheetInfo) {
					...
					this.resources.stylesheets.set(this.resources.stylesheets.size, { name, content: cssTree.generate(stylesheetInfo.stylesheet), url: stylesheetInfo.url });
				} else {
					linkElement.remove();
				}
			});
					...
					await Promise.all(urlFunctions.map(async urlFunction => {
						...
						if (!Util.testIgnoredPath(resourceURL)) {
							if (Util.testValidURL(resourceURL)) {
								...
								resources.backgroundImages.set(indexResource, { name, content, extension, contentType, url: resourceURL });
							}
						}
					}));
				}
