		...
		async run(onloadListener, options) {
			...
			return Promise.all(resourceURLs.map(async requestKey => {
				...
				try {
					...
					const extension = util.getFilenameExtension(resourceURL);
					resourceRequests.forEach(callbacks => callbacks.resolve({ content: content.data, indexResource, contentType: content.contentType, extension }));
				} catch (error) {
					indexResource = indexResource + 1;
					onloadListener({ url: resourceURL });
