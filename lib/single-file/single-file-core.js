...
class BatchRequest {
	...
	addURL(resourceURL, asBinary, expectedType) {
...
class Processor {
	...
	async getPageData() {
		...
		if (this.options.insertMetaCSP) {
			...
			metaTag.content = "default-src 'none'; font-src 'self' data: blob:; img-src 'self' data: blob:; style-src 'unsafe-inline' data: blob:; frame-src data: blob:; media-src 'self' data: blob:; script-src 'unsafe-inline' data: blob:;";
			...
			await Promise.all(ruleData.block.children.toArray().map(async declaration => {
				if (declaration.type == "Declaration" && declaration.value.children) {
					...
					await Promise.all(urlFunctions.map(async urlFunction => {
						...
						if (!testIgnoredPath(resourceURL)) {
							if (testValidURL(resourceURL)) {
								let { content, extension, indexResource, contentType } = await batchRequest.addURL(resourceURL, true, "font");
				...
				await Promise.all(urlFunctions.map(async urlFunction => {
					...
					if (!testIgnoredPath(resourceURL) || isDataURI) {
						if (testValidURL(resourceURL) || isDataURI) {
							let { content, indexResource, contentType, extension } = await batchRequest.addURL(resourceURL, true, "image");
			...
			if (resourceURL != null) {
				...
				if (!testIgnoredPath(resourceURL)) {
					...
					if (testValidPath(resourceURL)) {
						...
						if (testValidURL(resourceURL)) {
							let { content, indexResource, extension, contentType } = await batchRequest.addURL(resourceURL, true, "image");
							if (originURL) {
								if (!content) {
									try {
			...
			const srcsetValues = await Promise.all(srcset.map(async srcsetValue => {
				...
				if (!testIgnoredPath(resourceURL)) {
					if (testValidPath(resourceURL)) {
						...
						if (testValidURL(resourceURL)) {
							const { content, indexResource, extension, contentType } = await batchRequest.addURL(resourceURL, true, "image");
							const name = "images/" + indexResource + extension;
							resources.images.set(indexResource, { name, content, extension, contentType, url: resourceURL });
