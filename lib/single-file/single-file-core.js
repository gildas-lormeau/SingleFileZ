...
this.singlefile.lib.core = this.singlefile.lib.core || (() => {
		...
		async run(onloadListener, options) {
			...
			return Promise.all(resourceURLs.map(async requestKey => {
				...
				try {
					...
					if (!this.cancelled) {
						const extension = util.getFilenameExtension(resourceURL, options.filenameReplacementCharacter);
	...
	class Processor {
		...
		async getPageData() {
			...
			filename = util.getValidFilename(filename, this.options.filenameReplacementCharacter);
