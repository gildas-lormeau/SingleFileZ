...
this.singlefile.lib.core = this.singlefile.lib.core || (() => {
		...
		async run(onloadListener, options) {
			...
			return Promise.all(resourceURLs.map(async requestKey => {
				...
				try {
					...
					if (!this.cancelled) {
						let extension;
						if (content.contentType == CONTENT_TYPE_IMAGE_SVG) {
							extension = ".svg";
						} else {
							extension = util.getFilenameExtension(resourceURL, options.filenameReplacedCharacters, options.filenameReplacementCharacter);
						}
	...
	const PREFIX_DATA_URI_IMAGE_SVG = "data:" + CONTENT_TYPE_IMAGE_SVG + ",";
