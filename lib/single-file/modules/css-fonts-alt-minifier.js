...
this.singlefile.lib.modules.fontsAltMinifier = this.singlefile.lib.modules.fontsAltMinifier || (() => {
	...
	return {
		process: async (doc, stylesheets, fontResources) => {
			const fontsDetails = {
				fonts: new Map(),
				medias: new Map(),
				supports: new Map()
			...
			await Promise.all([...stylesheets].map(async ([, stylesheetInfo], sheetIndex) => {
				...
				if (cssRules) {
					if (media && media != MEDIA_ALL) {
						await processFontFaceRules(cssRules, sheetIndex, fontsDetails.medias.get("media-" + sheetIndex + "-" + media), fontResources, stats);
					} else {
						await processFontFaceRules(cssRules, sheetIndex, fontsDetails, fontResources, stats);
					}
					stats.rules.discarded -= cssRules.getSize();
				}
			}));
	...
	async function processFontFaceRules(cssRules, sheetIndex, fontsDetails, fontResources, stats) {
		const cssTree = singlefile.lib.vendor.cssTree;
		const removedRules = [];
		let mediaIndex = 0, supportsIndex = 0;
		for (let cssRule = cssRules.head; cssRule; cssRule = cssRule.next) {
			...
			if (ruleData.type == "Atrule" && ruleData.name == "media" && ruleData.block && ruleData.block.children && ruleData.prelude) {
				const mediaText = cssTree.generate(ruleData.prelude);
				await processFontFaceRules(ruleData.block.children, sheetIndex, fontsDetails.medias.get("media-" + sheetIndex + "-" + mediaIndex + "-" + mediaText), fontResources, stats);
				mediaIndex++;
			} else if (ruleData.type == "Atrule" && ruleData.name == "supports" && ruleData.block && ruleData.block.children && ruleData.prelude) {
				const supportsText = cssTree.generate(ruleData.prelude);
				await processFontFaceRules(ruleData.block.children, sheetIndex, fontsDetails.supports.get("supports-" + sheetIndex + "-" + supportsIndex + "-" + supportsText), fontResources, stats);
				supportsIndex++;
			} else if (ruleData.type == "Atrule" && ruleData.name == "font-face") {
				const fontInfo = fontsDetails.fonts.get(getFontKey(ruleData));
				if (fontInfo) {
					fontsDetails.fonts.delete(getFontKey(ruleData));
					await processFontFaceRule(ruleData, fontInfo, fontResources, stats);
				} else {
	...
	async function processFontFaceRule(ruleData, fontInfo, fontResources, stats) {
		const cssTree = singlefile.lib.vendor.cssTree;
		...
		const findSourceByFormat = (fontFormat, testValidity) => fontInfo.find(source => !source.src.match(EMPTY_URL_SOURCE) && (!source.contentType || !source.contentType.startsWith(CONTENT_TYPE_TEXT)) && !source.src.match(TEXT_CONTENT_URL_SOURCE) && source.format == fontFormat && (!testValidity || source.valid));
		const findSourceByContentType = (contentType, testValidity) => fontInfo.find(source => !source.src.match(EMPTY_URL_SOURCE) && !source.src.match(TEXT_CONTENT_URL_SOURCE) && source.contentType == contentType && (!testValidity || source.valid));
