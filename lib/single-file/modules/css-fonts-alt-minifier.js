...
this.singlefile.lib.modules.fontsAltMinifier = this.singlefile.lib.modules.fontsAltMinifier || (() => {
	...
	function processFontFaceRule(ruleData, fontInfo, fontResources, stats) {
		const cssTree = singlefile.lib.vendor.cssTree;
		const findSourceByFormat = fontFormat => fontInfo.find(source => !source.src.match(EMPTY_URL_SOURCE) && !source.contentType.startsWith(CONTENT_TYPE_TEXT) && !source.src.match(TEXT_CONTENT_URL_SOURCE) && source.format == fontFormat);
		const findSourceByContentType = contentType => fontInfo.find(source => !source.src.match(EMPTY_URL_SOURCE) && !source.src.match(TEXT_CONTENT_URL_SOURCE) && !source.contentType.startsWith(CONTENT_TYPE_TEXT) && source.contentType == contentType);
