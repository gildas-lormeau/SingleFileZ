...
this.singlefile.lib.modules.imagesAltMinifier = this.singlefile.lib.modules.imagesAltMinifier || (() => {
	...
	return {
		process(doc/*, imageResources*/) {
			...
			doc.querySelectorAll(":not(picture) > img[srcset]").forEach(imgElement => {
				let srcData = getImgSrcData(imgElement);
				// const srcset = singlefile.lib.vendor.srcsetParser.process(srcData.srcset);
				// srcset.forEach(info => imageResources.delete(Array.from(imageResources).find(([, resource]) => (resource.name == info.url))[0]));
				setSrc(srcData, imgElement, null);
			});
		}
	};
