...
singlefile.lib.processors.compression = (() => {
	...
	async function compressPage(pageData, options) {
		...
		if (browser && browser.runtime && browser.runtime.getURL) {
			zip.workerScriptsPath = "lib/single-file/vendor/zip/";
		...
		blobWriter.password = options.password;
		...
		await addPageResources(zipWriter, pageData, options.password, options.createRootDirectory ? String(Date.now()) + "_" + (options.tabId || 0) + "/" : "", options.url);
	...
	async function addPageResources(zipWriter, pageData, password, prefixName, url) {
		await addFile(zipWriter, prefixName, { name: "index.html", extension: ".html", content: pageData.content, url, password });
		const resources = {};
		...
		await addFile(zipWriter, prefixName, { name: "index.json", extension: ".json", content: jsonContent, password });
		for (const resourceType of Object.keys(pageData.resources)) {
			for (const data of pageData.resources[resourceType]) {
				if (resourceType == "frames") {
					await addPageResources(zipWriter, data, password, prefixName + data.name, data.url);
				} else {
					await addFile(zipWriter, prefixName, data);
				}
			}
		}
	}
	...
	async function addFile(zipWriter, prefixName, data) {
		...
		const options = { comment: data.url && data.url.startsWith("data:") ? "data:" : data.url, password: data.password };
	...
	function bootstrapCode() {
		...
		async function bootstrap(content) {
			...
			const zipReader = await new Promise((resolve, reject) => zip.createReader(blobReader, resolve, function (message) {
				alert("Error: " + message);
				reject(message);
			...
			for (const entry of entries) {
				...
				if (!blobReader.password && entry.passwordProtected) {
					blobReader.password = prompt("Please enter the password to view the page");
				}
