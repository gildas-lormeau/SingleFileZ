...
(() => {
	...
	history.pushState = function (state, title, url) {
		if (!warningDisplayed) {
			...
			warn.call(console, "SingleFileZ is hooking the history.pushState API to detect navigation."); // eslint-disable-line no-console		
		}
		try {
			dispatchEvent.call(window, new CustomEvent("single-filez-push-state", { detail: { state, title, url } }));
		} catch (error) {
			// ignored
		}
