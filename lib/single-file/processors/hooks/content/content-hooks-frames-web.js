...
(() => {
	...
	window.requestAnimationFrame = function (callback) {
		if (!warningRequestAnimationFrameDisplayed) {
			warn.call(console, "SingleFileZ is hooking the requestAnimationFrame and cancelAnimationFrame functions to load deferred images in background tabs."); // eslint-disable-line no-console
	...
	if (window.FontFace) {
		...
		window.FontFace = function () {
			if (!warningFontFaceDisplayed) {
				warn.call(console, "SingleFileZ is hooking the FontFace constructor to get font URLs."); // eslint-disable-line no-console
	...
	if (window.IntersectionObserver) {
		...
		window.IntersectionObserver = function () {
			if (!warningIntersectionObserverDisplayed) {
				warn.call(console, "SingleFileZ is hooking the IntersectionObserver API to detect and load deferred images."); // eslint-disable-line no-console
